	...
	if(isset($_POST['btnprio_1']))
	{
		...
		for ($x = 1; $x <= $count_1; $x++) {
			...
			if(isset($_POST['prio_1'.$x])) {
				...
				$belt=mysqli_query($con,"insert into tbl_top_coat_plan  (pro_no,p_date,design,lot,qty,order_no) values  ('$pro_no','$plan_date_fix','$design','$lot','$qty','$order') ")or die("insert_err4");
			} 
			else{
					...
					<td width="120" >
						<table width="120" height="75" border="0" align="center" cellpadding="0" cellspacing="0">
							...
							<tr class="tcday">
								...
								$query_1 =mysqli_query($con,"SELECT * FROM (SELECT a.*,b.actual_qty_qc from printing_status_tbl a
LEFT JOIN qc_approval_tbl b
ON a.ref_no_pst=b.pro_no_qc
WHERE a.pro06_pst='1'  GROUP BY production_no_pst ) aa
WHERE aa.production_no_pst NOT IN (select pro_no from tbl_top_coat_plan a ) ");
						...
						<tr>
							<td valign="middle" nowrap class="tdinfo">
								<input name="count_1" class="input3"id="count_1"					
							value="<?php print $count_1;?>" readonly hidden ></td>
							<td></td>
							<td></td>
							<td></td><td></td><td></td>
				...
				<td width="400" >
					<table width="400" height="100" border="0" align="center" cellpadding="0" cellspacing="0">
						...
						<tr class="tcday">
							...
							<td><input name="btnprio_2" value="Remove" class="button button1"  type="submit" src="search.jpg" width="60" height="20"    /></td>
							...
							$query_2 =mysqli_query($con,"select * from tbl_top_coat_plan a where p_date ='$today' order by ABS(order_no),pro_no");
