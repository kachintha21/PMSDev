...
<title>Ready To Test Plate</title>
<?php 
...
//$count_1new=isset($_REQUEST["count_1new"]) ? $_REQUEST["count_1new"] : "";
		...
		for ($x = 1; $x <= $count_1; $x++) {
			...
			header("Location:Ready_to_test_plate.php?p_date=$plan_date_fix");
	}
	...
	if(isset($_POST['btnprio_4']))
	{
		...
		header("Location:Ready_to_test_plate.php?p_date=$plan_date");
	}
...
<title>:: Noritake - Ready To Test Plate</title>
...
?>
	...
	<?php
		...
		<table width="800" height="5" border="0" align="center">
			<tr align="center">
				<td align="center" style="color:#36a2ef;font-size: -webkit-xxx-large;"><H1>Ready To Test Plate</H1></td>
			</tr>
		</table>
		<table width="800" height="5" border="0" align="center">
			<tr>
					...
					<td width="120" >
						<table width="120" height="75" border="0" align="center" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td colspan="5"  width="120" height="25" align="center" valign="middle" bgcolor="#cabd52" class="style15">Printing Approval Not Planned </td>
							...
							<tr class="style13">
								...
								<td width="10" height="25" align="left" valign="middle" bgcolor="#004848" class="style15">Shipping Date</td>
							...
							<tr class="tcday">
								...
								$query_1 =mysqli_query($con,"SELECT ref_no_pst as production_no_pst,pattern_no_pst AS pattern_no_pst,
lot_no_pst AS lot_no_pst,ship_scheduled_date_vpt FROM (SELECT a.*,b.actual_qty_qc,ttt.ship_scheduled_date_vpt from printing_status_tbl a
LEFT JOIN qc_approval_tbl b
ON a.ref_no_pst=b.pro_no_qc
LEFT JOIN (SELECT * FROM  virtual_planner_details_tbl d GROUP BY d.pro_no_vpt) ttt
ON a.ref_no_pst= ttt.pro_no_vpt
WHERE a.pro06_pst='1'  GROUP BY production_no_pst ) aa
WHERE aa.production_no_pst NOT IN (select pro_no from tbl_top_coat_plan a ) ORDER BY ship_scheduled_date_vpt");
								...
								<tr class="style13" <?php if($a_1['reset']=='1'){ ?> bgcolor="#CCFFFF"<?php }?>>
									...
									value="<?php print $a_1['ship_scheduled_date_vpt'];?>" readonly ></td>
					...
					<table width="120" height="75" border="0" align="center" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td colspan="5"  width="120" height="25" align="center" valign="middle" bgcolor="#cabd52" class="style15">Colour Finished </td>
							...
							<tr class="tcday">
								...
								$query_1 =mysqli_query($con,"select pro_no_lpt as production_no_pst,desing_no_lpt AS pattern_no_pst,
lot_no_lpt AS lot_no_pst,ttt.ship_scheduled_date_vpt FROM 
...
LEFT JOIN (SELECT * FROM  virtual_planner_details_tbl d GROUP BY d.pro_no_vpt) ttt
ON aaa.pro_no_lpt= ttt.pro_no_vpt
...
AND b.item01_vpft !='' AND b.item01_vpft >= DATE_SUB(NOW(), INTERVAL 500 DAY) AND b.status_vpft !='3')
group BY aaa.pro_no_lpt ORDER BY ttt.ship_scheduled_date_vpt  ");
