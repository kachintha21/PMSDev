...
<title>Inspection & Cut Daily Actual</title>
<?php 
...
//$count_1new=isset($_REQUEST["count_1new"]) ? $_REQUEST["count_1new"] : "";
		...
		for ($x = 1; $x <= $count_1; $x++) {
			...
			header("Location:inspection_daily_actual.php?p_date=$plan_date_fix");
	}
	...
	if(isset($_POST['btnprio_4']))
	{
		...
		header("Location:inspection_daily_actual.php?p_date=$plan_date");
	}
...
<title>:: Noritake - Inspection & Cut Daily Actual ::</title>
...
?>
	...
	<?php
		...
		<table width="800" height="5" border="0" align="center">
			<tr align="center">
				<td align="center" style="color:#36a2ef;font-size: -webkit-xxx-large;"><H3>Inspection & Cut Daily Actual</H3></td>
			</tr>
		</table>
		<table width="800" height="5" border="0" align="center">
			<tr>
					...
					<td width="120" >
						<table width="120" height="75" border="0" align="center" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td colspan="12"  width="120" height="25" align="center" valign="middle" bgcolor="#004848" class="style15">Inspection & Cut Daily Actual </td>
							...
							<tr class="tcday">
								...
								$query_1 =mysqli_query($con,"SELECT * FROM tbl_cut_ins_plan a
LEFT JOIN (SELECT c.pro_no_dt,c.planned_qty_dt,SUM(c.item01_dt)  act_qty
FROM dinspection_tbl  c GROUP BY c.pro_no_dt
) b ON  
a.pro_no = b.pro_no_dt
LEFT JOIN (SELECT e.pro_no_lpt,SUM(e.planned_qty_lpt) AS pln_qty FROM layout_plans_tbl e 
GROUP BY e.pro_no_lpt) d
ON 
a.pro_no=d.pro_no_lpt
WHERE a.p_date ='$today'");
								...
								<tr class="style13" <?php if($a_1['reset']=='1'){ ?> bgcolor="#CCFFFF"<?php }?>>
									...
									value="<?php print $a_1['pln_qty'];?>" readonly ></td>
									
									<td valign="middle" width="48" >
										<input name="qty<?php print $count_1 ?>"class="input3" id="qty<?php print $count_1 ?>"
										
									value="<?php print $a_1['act_qty'];?>" readonly ></td>
