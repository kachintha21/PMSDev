					...
					<li>
						...
						<table width="1049" height="82" border="1" cellpadding="0" cellspacing="0" class="display" align="center" id="tblData" >
							<tr>
								...
								<td width="61">Last Date Update</td>
								<td width="61">Decal Finished Plan Date</td>
						...
						<?php
							...
							$result1 =mysql_query("select * from (select *,(case when (aa.t is null) then aa.plan_n else aa.plan_n- aa.t - aa.def  end) AS `rem2` from (SELECT *, 
...
ORDER BY `aaa`.`pro_no_lpt`  " ); 
							...
							<tr onMouseOver="style.backgroundColor='#84DFC1'"  onmouseout="style.backgroundColor=''" >
								...
								$curves_per_sheet_qry =mysql_query("SELECT a.no_of_curves_lpt FROM layout_plans_tbl a WHERE a.pro_no_lpt='$pro_no'
AND a.curve_no_lpt ='$curve' limit 1") or die ("error3"); 

								if(mysql_num_rows($curves_per_sheet_qry)>0){
									while($curves_per_sheet_row=mysql_fetch_array($curves_per_sheet_qry))
									{
								...
								$sheet_qry =mysql_query("SELECT a.number_sheet_plt,a.printing_plt FROM preparing_layout_tbl a WHERE a.pro_no_plt='$pro_no'") or die ("error3"); 
									
									
								if(mysql_num_rows($sheet_qry)>0){
									while($sheet_row=mysql_fetch_array($sheet_qry))
									{
								...
								$no_of_colours_qry =mysql_query("SELECT a.* FROM colours_tbl a WHERE a.pattern_no_ct='$design_p'
AND a.colours_code_ct NOT LIKE 'T%'
GROUP BY a.colours_code_ct") or die ("error3"); 

								if(mysql_num_rows($no_of_colours_qry)>0){
									$no_of_colours = mysql_num_rows($no_of_colours_qry);
								...
								$current_stage_qry =mysql_query("SELECT a.plan_colors_vpft FROM virtual_planner_final_tbl a WHERE a.imp_actual_vpft=''
AND a.pro_no_vpft='$pro_no' ORDER BY id LIMIT 1") or die ("error3"); 

								if(mysql_num_rows($current_stage_qry)>0){
									
									while($current_stage_row=mysql_fetch_array($current_stage_qry))
									{
										...
										if($current_stage == 'S01'){
											...
											$current_stage_qry3 =mysql_query("SELECT  a.item01_vpft FROM virtual_planner_final_tbl a WHERE a.pro_no_vpft='$pro_no' 
AND a.plan_colors_vpft='$pre_ref'") or die ("error3"); 

											if(mysql_num_rows($current_stage_qry3)>0){
											while($current_stage_row3=mysql_fetch_array($current_stage_qry3))
											{
									...
									$current_stage_qry2 =mysql_query("SELECT a.plan_colors_vpft FROM virtual_planner_final_tbl a WHERE a.imp_actual_vpft !=''
AND a.pro_no_vpft='$pro_no' ORDER BY id LIMIT 1") or die ("error3"); 

									if(mysql_num_rows($current_stage_qry2)>0){
										
										$current_stage_qry4 =mysql_query("SELECT  * FROM qc_approval_tbl a WHERE a.pro_no_qc='$pro_no' ") or die ("error3"); 

										if(mysql_num_rows($current_stage_qry4)>0){
											
											$current_stage_qry6 =mysql_query("SELECT a.date FROM tbl_top_coat_plan a WHERE a.pro_no='$pro_no'  ") or die ("error3"); 
											
											if(mysql_num_rows($current_stage_qry6)>0){
												
												while($current_stage_row6=mysql_fetch_array($current_stage_qry6))
												{
												...
												$current_stage_qry7 =mysql_query("SELECT a.org_date_tcpt FROM top_coat_printing_tbl a WHERE a.pro_no_tcpt='$pro_no'   ") or die ("error3"); 
											
												if(mysql_num_rows($current_stage_qry7)>0){
													
													while($current_stage_row7=mysql_fetch_array($current_stage_qry7))
													{
														$top_act_date = $current_stage_row7[0];
														
														$current_stage_qry9 =mysql_query("SELECT a.org_date_qc FROM qc_qc_approval_tbl a WHERE a.pro_no_qc='$pro_no'  ") or die ("error3"); 
											
														if(mysql_num_rows($current_stage_qry9)>0){
															
															while($current_stage_row9=mysql_fetch_array($current_stage_qry9))
															{
																$qc_act_date = $current_stage_row9[0];
															}
															print "<td>".("QC Approved")."</td>";
												...
												$current_stage_qry8 =mysql_query("SELECT a.org_date_pst FROM printing_status_tbl a WHERE a.pro06_pst='1' and a.production_no_pst ='$pro_no' GROUP BY a.production_no_pst    ") or die ("error3"); 
											
												if(mysql_num_rows($current_stage_qry8)>0){
													while($current_stage_row8=mysql_fetch_array($current_stage_qry8))
													{
											...
											$current_stage_qry5 =mysql_query("SELECT a.item01_vpft FROM virtual_planner_final_tbl a WHERE a.pro_no_vpft='$pro_no' 
AND a.item01_vpft !='' ORDER BY id DESC LIMIT 1") or die ("error3"); 

											if(mysql_num_rows($current_stage_qry5)>0){
											while($current_stage_row5=mysql_fetch_array($current_stage_qry5))
											{
								...
								$decal_finish_plan_date_qry =mysql_query("SELECT a.date_vpft FROM virtual_planner_final_tbl a WHERE a.pro_no_vpft='$pro_no' ORDER BY id DESC LIMIT 1") or die ("error3"); 
									
									if(mysql_num_rows($decal_finish_plan_date_qry)>0){
										while($decal_finish_plan_date_row=mysql_fetch_array($decal_finish_plan_date_qry))
										{
								...
								$top_coat_plan_date_qry =mysql_query("SELECT a.p_date FROM tbl_top_coat_plan a WHERE a.pro_no='$pro_no' limit 1") or die ("error3"); 
									
									if(mysql_num_rows($top_coat_plan_date_qry)>0){
										while($top_coat_plan_date_row=mysql_fetch_array($top_coat_plan_date_qry))
										{
