...
#left {
 ...
 .myButton2 {
        ...
        background:-webkit-gradient(linear, left top, left bottom, color-stop(0.05, #666600), color-stop(1, #408c99));
        background:-moz-linear-gradient(top, ##666600 5%, #408c99 100%);
        background:-webkit-linear-gradient(top, ##666600 5%, #408c99 100%);
        background:-o-linear-gradient(top, ##666600 5%, #408c99 100%);
        background:-ms-linear-gradient(top, ##666600 5%, #408c99 100%);
        background:linear-gradient(to bottom, ##666600 5%, #408c99 100%);
        filter:progid:DXImageTransform.Microsoft.gradient(startColorstr='##666600', endColorstr='#408c99',GradientType=0);
...
and table_name NOT LIKE '%temp%'")or die('err');  
 while ($row = mysql_fetch_row($result)) {
	 
	echo "<option value=daily_plan_plan.php?sdate=".substr($row[0],5).">.".substr($row[0],5)."</option>"; 
 }
    if(isset($_GET['sdate']))
	{ 
	echo "<option value=daily_plan_plan.php?sdate=".$_GET['sdate']." selected>.".$_GET['sdate']."</option>"; 
	}
	?>
  </select>
  </span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href ='../../sort_table.php?type=belt' class="myButton" onclick= "var result = confirm('really?'); if (result==true) return true; else return false;">Sort Belt</a> <a href ='../../sort_table.php?type=pjt' class="myButton" onclick= "var result = confirm('really?'); if (result==true) return true; else return false;">Sort PJT</a>
  <a href ='../../sort_table.php?type=pattern' class="myButton" onclick= "var result = confirm('really?'); if (result==true) return true; else return false;">Sort Pattern</a>
  <a href ='../../sort_table.php?type=item' class="myButton" onclick= "var result = confirm('really?'); if (result==true) return true; else return false;">Sort Item</a>
  
    <a href ='getTable2.php' class="myButton2"  target="_new">Shipment Vs Design / Item </a>
</form>
 <div id="left">
 <div id="container">
  ...
  return function () {
    ...
    return part;
            ...
            $container.handsontable({
			  ...
			  colWidths: [100, 50, 60, 100, 100, 100, 60, 80,50, 100, 110,100],
              startRows: 8,
              startCols: 12,
               rowHeaders: true,
              colHeaders: [ 'Plan Date','Belt','Distant', 'PJT No', 'Pattern', 'Item', 'Plan Qty', 'Kiln','Priority', 'Remarks', 'Shipment',  'Ship Date'],
    ...
    {
     	//8 
		source: [ "RHK I","RHK III", "RHK IV", "RHK V","D"],
	 type: 'autocomplete',  
    },
	{
      type: 'autocomplete',
