<?php
	...
	if($mode =='final'){
		...
		$delete=mysqli_query($conn,"delete from virtua_time_store_tbl where date_vtst='$sdate' ");
		
		$insert=mysqli_query($conn,"INSERT INTO virtua_time_store_tbl select '',ref_no_vtst, pro_no_vtst, lot_vtst, num_colors_vtst, plan_colors_vtst,machine_no_vtst,date_vtst,total_time_vtst,qty_vtst,
		status_vtst,item01_vtst,item02_vtst,'','','','','','',''
		from virtua_time_store_tbl_temp ") ;
		...
		if($sdate != null){
			...
			if($mode =='start'){
				...
				if($t_plan != $sdate){
					//print($sdate);exit;
					$truncate=mysqli_query($conn,"TRUNCATE TABLE virtua_time_store_tbl_temp");
					
					$insert=mysqli_query($conn,"INSERT INTO virtua_time_store_tbl_temp  select *
					from virtua_time_store_tbl  where date_vtst='$sdate' ORDER BY id ASC") ;			
				}
				...
				$truncate=mysqli_query($conn,"TRUNCATE TABLE virtua_time_store_tbl_temp_sort");
				$insert=mysqli_query($conn,"INSERT INTO virtua_time_store_tbl_temp_sort select *
				from virtua_time_store_tbl_temp ORDER BY $type ASC") ;
				
				$truncate=mysqli_query($conn,"TRUNCATE TABLE virtua_time_store_tbl_temp");
				$insert=mysqli_query($conn,"INSERT INTO virtua_time_store_tbl_temp select *
				from virtua_time_store_tbl_temp_sort   ORDER BY $type ASC") ;
								...
								<div class="pad">
									<script>
										...
										$container.handsontable({
											...
											colWidths: [80,60, 50, 50, 50, 50, 100, 80, 50,50, 150,100],
											startRows: 8,
											startCols: 12,
											rowHeaders: true,
											colHeaders: [ 'Ref no','Pro no','Lot', 'NoC', 'Colour', 'Mc No', 'Plan Date', 'Total Time','Qty','Status', 'Remarks','Pattern'],
											...
											$.ajax({
												...
												success: function (res) {console.log(res);
													...
													for (var i = 0, ilen = res.daily_plan.length; i < ilen; i++) {
														...
														row[0] = res.daily_plan[i].ref_no_vtst ;
														row[1] = res.daily_plan[i].pro_no_vtst;
														
														row[2] = res.daily_plan[i].lot_vtst;
														row[3] = res.daily_plan[i].num_colors_vtst;
														
														row[4] = res.daily_plan[i].plan_colors_vtst;
														row[5] = res.daily_plan[i].machine_no_vtst;
														
														row[6] = res.daily_plan[i].date_vtst;
														row[7] = res.daily_plan[i].total_time_vtst ;
														row[8] = res.daily_plan[i].qty_vtst;
														row[9] = res.daily_plan[i].status_vtst;
														
														row[10] = res.daily_plan[i].item01_vtst ;
														row[11] = res.daily_plan[i].item02_vtst;
