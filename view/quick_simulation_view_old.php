...
include_once '../include/Planner/PrintPlanner_old.php';
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>NLPL  Layout Simulator| <?php
            echo($id);
            ?></title>
        <style>
            .center {
                                        ...
                                        <td colspan="3" align="center">
                                            ...
                                            try {
                                                ...
                                                if (isset($_POST['save_plans']) && isset($_SESSION['selectedPlans']) && count($_SESSION['selectedPlans']) > 0) {
                                                    ...
                                                    $stmt = $conn->prepare("INSERT INTO saved_layout_plans_tbl (`ref_id`, layout, design,schedule,curve_no, curve_area, planned_qty, no_of_curves, no_of_sheets, close_curve_after, total_sheets_needed) VALUES (?, ?, ?, ?,?, ?, ?, ?, ?, ?,?) ");
                                                                ...
                                                                foreach ($plans as $plan) {
                                                                    ...
                                                                    if (is_array($plan['curves_sets'])) {
                                                                        foreach ($plan['curves_sets'] as $v) {
                                                                            ...
                                                                            $stmt->bind_param(
                                                                                    "ssssdiiiiii", $refId, $layout, $design_fdt, $schedule_date, $v['curve_no'], $v['curve_area'], $v['planned_qty'], $v['curves_per_sheet'], $v['no_of_sheets'], $v['close_curve_after'], $plan['total_no_of_sheets']
                                                                            );
