...
//                        }
    ...
    public function getDailyOutPutQtyMachineCalendar($date, $mname) {
        $query = "SELECT org_date_fot,COUNT(*) AS t FROM decal_ins_cycletime_tbl WHERE org_date_fot='" . $date . "' AND judgment_fot='OK'  AND  pattern_no_fot='" . $mname . "'  ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
    ...
    $id, $arrange, $ins, $seal, $datain, $is_edit_mlt, $item01_mlt, $item02_mlt, $item03_mlt, $item04_mlt, $item05_mlt, $org_name_mlt, $org_date_mlt, $org_time_mlt
    ) {


        $res = "INSERT INTO decal_ins_cycletime_tbl  SET
       id='" . $id . "',
        arrange='" . $arrange . "',
		ins='" . $ins . "',
		seal='" . $seal . "',
    ...
    $id, $date_mlt, $machine_no_mlt, $number_shift_mlt, $work_time_mlt, $is_edit_mlt, $item01_mlt, $item02_mlt, $item03_mlt, $item04_mlt, $item05_mlt, $org_name_mlt, $org_date_mlt, $org_time_mlt
        ...
        $res = "UPDATE decal_ins_cycletime_tbl SET   	    
            date_mlt='" . $date_mlt . "',
    ...
    public function deleteMachineCalendar($id) {
        $query = "DELETE FROM decal_ins_cycletime_tbl WHERE id='$id'";
        $result = $this->mysqli->query($query);
        if ($result) {
