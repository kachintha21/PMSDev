...
class DecalInsCalendar {
    ...
    public function __destruct() {
        ...
        $la = array();
        $query = "SELECT*FROM decal_ins_calendar_tbl WHERE id='" . $id . "'  ";
    ...
    public function getPlanedQtyByNo($id) {
        $la = array();
        $query = "SELECT*FROM decal_ins_calendar_tbl WHERE pro_no_plt='" . $id . "'  ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
            while ($rows = $result->fetch_assoc()) {
    ...
    public function getMachineCalendarByDate($date_mlt, $machine_no_mlt) {
        $la = array();
        $query = "SELECT*FROM decal_ins_calendar_tbl WHERE machine_no_mlt='" . $machine_no_mlt . "'   AND  date_mlt='" . $date_mlt . "'  ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
            while ($rows = $result->fetch_assoc()) {
    ...
    public function getMachineCalendarByNo($id) {
        $la = array();
        $query = "SELECT*FROM decal_ins_calendar_tbl WHERE pro_no_sm='" . $id . "'  ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
            while ($rows = $result->fetch_assoc()) {
    ...
    public function getMachineCalendarTimeCheck($machine_no_vtst, $date_vtst, $is) {
        $la = array();
        $query = "SELECT*FROM decal_ins_calendar_tbl WHERE machine_no_mlt='" . $machine_no_vtst . "'  AND   date_mlt='" . $date_vtst . "' ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
            while ($rows = $result->fetch_assoc()) {
...
//                        }
    ...
    public function getDailyOutPutQtyMachineCalendar($date, $mname) {
        $query = "SELECT org_date_fot,COUNT(*) AS t FROM decal_ins_calendar_tbl WHERE org_date_fot='" . $date . "' AND judgment_fot='OK'  AND  pattern_no_fot='" . $mname . "'  ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
    ...
    $id, $date_mlt, $ins_no_1, $ins_no_2, $ins_no_3, $ins_no_4, $ins_no_5, $ins_no_6, $is_edit_mlt, $item01_mlt, $item02_mlt, $item03_mlt, $item04_mlt, $item05_mlt, $org_name_mlt, $org_date_mlt, $org_time_mlt
        ...
        $res = "INSERT INTO decal_ins_calendar_tbl  SET
       id='" . $id . "',
        date_mlt='" . $date_mlt . "',
        ins_no_1='" . $ins_no_1 . "',
		ins_no_2='" . $ins_no_2 . "',
    ...
    $id, $date_mlt, $machine_no_mlt, $number_shift_mlt, $work_time_mlt, $is_edit_mlt, $item01_mlt, $item02_mlt, $item03_mlt, $item04_mlt, $item05_mlt, $org_name_mlt, $org_date_mlt, $org_time_mlt
        ...
        $res = "UPDATE decal_ins_calendar_tbl SET   	    
            date_mlt='" . $date_mlt . "',
    ...
    public function deleteMachineCalendar($id) {
        $query = "DELETE FROM decal_ins_calendar_tbl WHERE id='$id'";
        $result = $this->mysqli->query($query);
        if ($result) {
