...
class Replan {
    ...
    public function getColorSeparationByNo($id) {
        $la = array();
        $query = "SELECT*FROM replan_tbl WHERE pro_no_plt='" . $id . "'  ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
            while ($rows = $result->fetch_assoc()) {
    ...
    public function getDailyOutPutQtyColorSeparation($date, $mname) {
        $query = "SELECT org_date_sc,COUNT(*) AS t FROM replan_tbl WHERE org_date_sc='" . $date . "' AND judgment_sc='OK'  AND  pattern_no_sc='" . $mname . "'  ";
        $result = $this->mysqli->query($query);
        $num_result = $result->num_rows;
        if ($num_result > 0) {
    ...
    public function confirmColorSeparationEntity($ref_no_plt) {
        $res = "SELECT*FROM replan_tbl WHERE ref_no_plt = '" . $ref_no_plt . "'   ";
    ...
    $id, $pro_no_rp, $lot_no_rp, $fg_design_no_rp, $curve_no_rp, $decal_design_no_rp, $plan_qty_rp, $actual_qty_rp, $is_plan_date_rp, $is_actual_date_rp, $is_edit_rp, $item01_rp, $item02_rp, $item03_rp, $item04_rp, $item05_rp, $org_name_rp, $org_date_rp, $org_time_rp
    ) {



        $res = "INSERT INTO replan_tbl  SET
             id='" . $id . "',
            pro_no_rp='" . $pro_no_rp . "',
            lot_no_rp='" . $lot_no_rp . "',
            fg_design_no_rp='" . $fg_design_no_rp . "',
            curve_no_rp='" . $curve_no_rp . "',
            decal_design_no_rp='" . $decal_design_no_rp . "',
            plan_qty_rp='" . $plan_qty_rp . "',
            actual_qty_rp='" . $actual_qty_rp . "',
            is_plan_date_rp='" . $is_plan_date_rp . "',
            is_actual_date_rp='" . $is_actual_date_rp . "',
            is_edit_rp='" . $is_edit_rp . "',
            item01_rp='" . $item01_rp . "',
            item02_rp='" . $item02_rp . "',
            item03_rp='" . $item03_rp . "',
            item04_rp='" . $item04_rp . "',
            item05_rp='" . $item05_rp . "',
            org_name_rp='" . $org_name_rp . "',
            org_date_rp='" . $org_date_rp . "',
            org_time_rp='" . $org_time_rp . "'
    ...
    $id, $pro_no_rp, $lot_no_rp, $fg_design_no_rp, $curve_no_rp, $decal_design_no_rp, $plan_qty_rp, $actual_qty_rp, $is_plan_date_rp, $is_actual_date_rp, $is_edit_rp, $item01_rp, $item02_rp, $item03_rp, $item04_rp, $item05_rp, $org_name_rp, $org_date_rp, $org_time_rp
        ...
        $res = mysql_query("UPDATE replan_tbl SET
                pro_no_rp='" . $pro_no_rp . "',
                lot_no_rp='" . $lot_no_rp . "',
                fg_design_no_rp='" . $fg_design_no_rp . "',
                curve_no_rp='" . $curve_no_rp . "',
    ...
    public function deleteColorSeparation($id) {
        $query = "DELETE FROM replan_tbl WHERE id='$id'";
        $result = $this->mysqli->query($query);
        if ($result) {
