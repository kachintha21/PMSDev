function form_save() {
	...
	$j('#section_name_st').removeClass('error');
	 ...
	 if($j('#section_name_st').val() == '') {
        $j('#section_name_st').addClass('error');
        valid = false;
        str += '<li>section name can  not be empty.</li>';
    }	
	...
	if(valid){
		...
		$j.post(
			"../controllers/section_process.php", 
			{
				form_data: $j( "form" ).serialize()
			},
			function(data) {
				if(parseInt(data) == 1){
					window.location="section_view.php";
				}
				else{
...
function submit_account() {
	...
	if(valid){
		$j('#insert_response').html("<img  src='../img/ajax-loader-3.gif'/>");
		$j.post(
			"../controllers/account_managed_process.php", 
			{
				form_data: $j( "form" ).serialize()
			},
			function(data) {
				if(parseInt(data) == 1){
					window.location="account_managed_view.php";
				}
				else{
					$j('#insert_response').html(data);
				}
...
function submit_process() {
    ...
    var valid = true;
    var str = '<ul>';
	$j('#process_name_pt').removeClass('error');

    
	 if($j('#process_name_pt').val() == '') {
        $j('#process_name_pt').addClass('error');
        valid = false;
	...
	if(valid){
		$j('#insert_response').html("<img  src='../img/ajax-loader-3.gif'/>");
		$j.post(
			"../controllers/process_process.php", 
			{
				form_data: $j( "form" ).serialize()
			},
			function(data) {
				if(parseInt(data) == 1){
					window.location="process_view.php";
				}
				else{
					$j('#insert_response').html(data);
				}
