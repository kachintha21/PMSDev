function  form_save() {
    ...
    $j('#yield_value_ymt').removeClass('error');
	 ...
	 if($j('#decal_design_no_cmt').val() == '') {
        ...
        str += '<li>decal design no not be empty.</li>';
    	 ...
    	 if($j('#yield_value_ymt').val() == '') {
        $j('#yield_value_ymt').addClass('error');
        valid = false;
        str += '<li>yield_value_ymt can not be empty.</li>';
    }
    ...
    if (valid) {
        ...
        $j.post(
                "../controllers/yield_process.php",
                {
                    form_data: $j("form").serialize()
                },
        function (data) {
            if (parseInt(data) == 1) {
                window.location = "yield_view.php";
            }
            else {
