...
class CurveMaster {
    ...
    public function getCurveAreaById($id) {
        ...
        if ($num_result > 0) {
            while ($rows = $result->fetch_assoc()) {
               array_push($la, $rows['id'], $rows['fg_design_no_cmt'], $rows['curve_no_cmt'], $rows['decal_design_no_cmt'], $rows['curve_area_cmt'], $rows['is_edit_cmt'], $rows['org_date_cmt'], $rows['org_time_cmt']
                );
            }
            return $la;
        }
