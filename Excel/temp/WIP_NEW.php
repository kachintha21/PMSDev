...
<select name="belt" id="belt">
  ...
  <select name="shipment" id="shipment">
    <option value="">All</option>
    <?php
    $sql1 =mysql_query("SELECT distinct shipment FROM daily_plan2 ORDER BY shipment ASC"); 
...
<div class="CSSTableGenerator" align="center" >
 ...
 <td width="59">Date</td>
	...
	if(isset($_POST['filter']))
	{
	   $sql =mysql_query("SELECT * FROM cm  INNER JOIN daily_plan2  ON cm.pjt_no = daily_plan2.pjtno AND design=pattern AND item=itemno WHERE $where AND belt !='D' AND plan_status='0'  ORDER BY cm.id,design, item,chit_no  ")or die ("err"); 
	 }
