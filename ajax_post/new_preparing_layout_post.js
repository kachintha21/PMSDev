...
function  getData() {
    ...
    str += '</ul>';
    if (!valid) {
        $j('#insert_response_data').html(str);
        $j('#insert_response_data').addClass('error');
    ...
    if (valid) {
        //$j('#insert_response').html("<img  src='../img/ajax-loader-3.gif'/>");
        $j.post(
                "../controllers/simulation_respone_process.php",
                {
                    form_data: $j("form").serialize()
                },
        function (data) {
            if (parseInt(data) == 1) {
                window.location = "preparing_layout.php";
            }
            else {
                $j('#insert_response_data').html(data);
            }
...
function  form_ps() {
    ...
    if (valid) {
        $j('#insert_response').html("<img  src='../img/ajax-loader-3.gif'/>");
        $j.post(
                "../controllers/decal_details_reg_ps_process.php",
                {
                    form_data: $j("form").serialize()
                },
        function (data) {
